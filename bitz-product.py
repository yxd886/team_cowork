...
def tick():
    ...
    api = bit_api(access_key, access_secret,tradePwd)

    coin_amount = float(entry6.get())

    step = float(entry7.get())/2

    coin_should_have = float(entry8.get())
    ...
    while True:
        try:
            time.sleep(2)
            current_coin_step_for_buy = coin_amount

            current_coin_step_for_sell = coin_amount
        ...
        while True:
            try:
                ...
                print("交易次数:%d" % trade_time)
                print("buy order:%s"%buy_order_id)
                print("sell_order:%s" % sell_order_id)
...
if __name__ == '__main__':
    ...
    label = tkinter.Label(win, text="请在下面输入框输入您的交易密码:")
    label.pack()
    ...
    label = tkinter.Label(win, text="请输入每次交易虚拟货币数量：")
    label.pack()
    entry6 = tkinter.Entry(win, width=50, bg="white", fg="black")
    entry6.pack()

    label = tkinter.Label(win, text="请输入挂单差价：")
    label.pack()
    entry7 = tkinter.Entry(win, width=50, bg="white", fg="black")
    entry7.pack()

    label = tkinter.Label(win, text="请输入您当前要求的虚拟货币持仓量：")
    label.pack()
