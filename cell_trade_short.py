...
def cancel_order(order_id, instrument_id):
    base_url = 'https://www.okex.com'
    request_path = '/api/swap/v3/cancel_order/' + instrument_id + '/' + order_id
    # request params
...
def get_order_pending():
    base_url = 'https://www.okex.com'
    request_path = '/api/swap/v3/orders_pending'
    url = base_url + request_path
    now = get_time_stamp()
    header = get_header(apiKey, signature(now, 'GET', request_path, None, secreetKey), now, 'dxyDXY835472123')
    response = requests.get(url, headers=header)
...
def get_account_info(instrument_id):
    base_url = 'https://www.okex.com'
...
def get_available_balance(currency):
    ...
    margin_for_unfilled = float(response['margin'])
...
def get_current_price(instrument_id):
    ...
    sellPrice = float(depth['bids'][0][0])
    return sellPrice



def take_market_order(instrument_id, direction, size):
    # take order
    base_url = 'https://www.okex.com'
    request_path = '/api/swap/v3/order'
    type = '0'
...
def get_order_info(instrument_id,order_id):
    base_url = 'https://www.okex.com'
    request_path = '/api/swap/v3/orders/' + instrument_id + '/'+order_id
    ...
    response = requests.get(url, headers=header)
    print(response.json())
    price_avg = float(response.json().get('price',0))
    return price_avg

price_per_sheet = 10
price_per_iteration = 10

step = int(price_per_iteration/price_per_sheet)
instrument_id="EOS-USD-SWAP"
buy_ratio = 0.002
sell_ratio = 0.001

order_price =3.641
buyPrice = order_price*(1-buy_ratio)
...
while True:
    try:
        ...
        if current_price<order_price*(1-buy_ratio):
            order_price = order_price*(1-sell_ratio)
        ratio = (current_price-order_price)/order_price
        print("order_price:%f,current_price:%f"%(order_price,current_price))
        print(ratio)
        if current_price>order_price and ratio>=sell_ratio:
            sell_order_id = take_market_order(instrument_id, "sell", step);
            if sell_order_id != '-1':
                order_price = get_order_info(instrument_id,sell_order_id)
                while order_price==0:
                    time.sleep(1)
                    order_price = get_order_info(instrument_id,sell_order_id)
                buyPrice = order_price*(1-buy_ratio)
                take_order(instrument_id, "buy", buyPrice, step);
    except:
        continue
    else:
        continue
